...
export const ConsolePanel: React.FC = () => {
    ...
    const { isInSession, isTeacher, studentOutput, studentTestResults } = useSession();
    const bottomRef = useRef<HTMLDivElement>(null);
    const [activeTab, setActiveTab] = useState<'terminal' | 'tests'>('terminal');

    // Use student data if teacher is viewing
    const displayOutput = isInSession && isTeacher ? studentOutput : output;
    const displayTestResults = isInSession && isTeacher ? studentTestResults : testResults;
    ...
    }, [displayOutput, activeTab]);

    const getLineStyle = (line: string) => {
        if (line.startsWith('❌') || line.startsWith('Error:') || line.includes('Traceback')) return 'text-red-400';
        if (line.startsWith('>>>')) return 'text-blue-400';
        return 'text-slate-200';
    };

    const passedCount = displayTestResults?.filter(r => r.passed).length || 0;
    const totalTests = displayTestResults?.length || 0;
    ...
    return (
        <div className="flex flex-col h-full bg-[#0d0d0d] font-mono text-sm">
            ...
            <div className="flex items-center bg-[#1a1a1a] border-b border-[#2a2a2a]">
                ...
                >
                    Terminal
                    {isInSession && isTeacher && <span className="ml-1 text-[10px] text-purple-400">(Student)</span>}
                </button>
                <button
                ...
                >
                    Test Cases
                    {isInSession && isTeacher && <span className="text-[10px] text-purple-400">(Student)</span>}
                    {displayTestResults && (
                ...
                {activeTab === 'terminal' ? (
                    ...
                    <div className="space-y-3">
                        {!displayTestResults ? (
                            <div className="text-slate-600 italic text-xs">
                                {isInSession && isTeacher
                                    ? "Waiting for student to run tests..."
                            ...
                            <>
                                ...
                                <div className={`p-3 rounded-lg border ${allPassed
                                    ...
                                    <div className="text-xs opacity-80">
                                        {isInSession && isTeacher ? 'Student passed' : 'You passed'} {passedCount} out of {totalTests} tests.
                                    </div>
                                </div>
